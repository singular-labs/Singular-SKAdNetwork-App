...
- (IBAction)showTrackingConsentDialog:(id)sender {
    ...
    // The `trackingAuthorizationStatus` persist the result of the Tracking Consent dialog and can only be changed through the settings app.
    // Tracking Consent dialog is only shown once per install, meaning that call `requestTrackingAuthorizationWithCompletionHandler` won't show the dialog again.
    if ([ATTrackingManager trackingAuthorizationStatus] != ATTrackingManagerAuthorizationStatusNotDetermined){
        [self alertTrackingConsentIsAlreadySet];
    }
    
    // Checking the OS version before calling the Tracking Consent dialog, it's available only in v14.0 and above
    if (@available(iOS 14, *)) {
        ...
        // No need for to display the dialog in earlier versions
    }
}

- (IBAction)updateConversionValueClick:(id)sender {
    // Once `registerAppForAdNetworkAttribution` is called for the first time (check the AppDelegate.m for explanations on `registerAppForAdNetworkAttribution`),
    // A 24 hours window is opened the check for attribution data. Using `updateConversionValue` we can add a value (number between 0-63) to be sent with the attribution notification.
...
- (void)alertTrackingConsentIsAlreadySet {
    UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"Can't Display Dialog"
                                                                   message:@"Can't display Tracking Consent dialog because it was already been displayed. "
